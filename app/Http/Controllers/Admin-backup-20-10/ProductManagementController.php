...
class ProductManagementController extends Controller
{
    ...
    {
    	$arr_data = $arr_resp = [];
    	$obj_data = $this->SystemCityModel->get();
	...
	{	
		...
		if(isset($build_result->data) && sizeof($build_result->data)>0)
		{
			foreach ($build_result->data as $key => $data) 
			{
				...
				{
					$build_status_btn = '<a  href="'.$this->module_url_path.'/unblock/'.base64_encode($data->id).'" onclick="return confirm_action(this,event,\'Do you really want to activate this Product ?\')"><i class="fa fa-eye" title="Blocked"></i></a>&nbsp&nbsp&nbsp ';
				}
				elseif($data->status != null && $data->status == "1")
				{
					$build_status_btn = '<a  href="'.$this->module_url_path.'/block/'.base64_encode($data->id).'" onclick="return confirm_action(this,event,\'Do you really want to inactivate this Product ?\')"><i class="fa fa-eye-slash" title="Active"></i></a> &nbsp&nbsp&nbsp';
                }
...
// 			dd($arr_product_option);
	...
	public function product_weight_time_cost($product_id)
    {
    	...
    	$obj_delete_two = $this->ProductWeightTimeCostDetailsModel->where('product_id',$product_id)->delete();
			...
			foreach ($arr_option as $key_option => $value_option) {
				...
				foreach ($arr_product_sub_option as $key_sub_option => $value_sub_option) {
					...
					if($value_option==$value_sub_option['option_id']){
						${'arr_sub_option_' . $key_option}[] = $value_sub_option['sub_option_id'];   //created dynamic array variable as options are dynamic
					}
				}
			}
		}
		...
		for($i=0;$i<sizeof($arr_product_option);$i++){
			$arrays[] = ${'arr_sub_option_' . $i};   
		}	

		if(sizeof($arrays)>1){
        // dump($arrays);
			$arr_of_combinations 	=	 $this->combinations($arrays);
...
// 			}
            ...
            // New code added on 14th Sept 2020 for slide 11 feedback
				foreach ($arrays[0] as $key_c4 => $value_c4) {
