...

{
	...
	{	
...

		{
...

			{
				...
				$action_button_html = '<a  title="" href="'.$view_link_url.'" data-original-title="View" data-id="'.$data->id.'" id="open_edit_evaluation_modal"><i class="icon-pencil7" title="Edit"></i></a>';
				...
				if($data->status =='no_comments'){
					...
					$status = '<span class="label label-primary">On Comment</span>';
					...
					$status = '<span class="label label-danger update-status" status="rejected" rel="'.$id.'" >Reject</span>
					<span class="label label-success update-status" status="approved" rel="'.$id.'" >Approve</span>';
	...
	public function change_evalution_status(Request $request)
	{	$arr_resp = array();
		$arr_resp['status'] 	= 'error';
		$status = $request->input('status');	
		$id = $request->input('id');
		if($id){	
			$obj_check = $this->BaseModel->where('id',$id)->update(['status'=>$status]);
			if($obj_check){
				$arr_resp['status']  	= 'success';
			}else{
				$arr_resp['status'] 	= 'error';
