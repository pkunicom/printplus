...
class AgentManagementController extends Controller
{
	...
	{	
		...
		if(isset($build_result->data) && sizeof($build_result->data)>0)
		{
			foreach ($build_result->data as $key => $data) 
			{
				...
				$delete_button_html = '&nbsp;&nbsp;&nbsp;&nbsp;<a class="dropdown-item" href='.$delete_link_url.'  title="Delete" onclick="return confirm_action(this,event,\'Do you really want to delete this User ?\')"><i class="fa fa-trash"></i></a>';
				...
				$build_result->data[$key]->built_action_btns    = $action_button_html."   ".$delete_button_html;
    ...
    // get all cities data: AUTHOR (Harsh chauhan)
    public function check_duplicate_number($enc_id)
    {
    	$mobile_no = ($enc_id);
    	$arr_data = $arr_resp = [];
    	$obj_data = $this->AgentContactDetailsModel->where('mobile_one',$mobile_no)->first();
		...
		if($arr_data){
			$arr_resp['status']  	= 'error';
			$arr_resp['data'] 		=  'Mobile Number already Exist';
		}else{
			$arr_resp['status'] 	= 'success';
			$arr_resp['data'] 		=  'Valid';
		}
