...
trait MultiActionTrait
{
    ...
    public function unblock($enc_id = FALSE,Request $request)
    {
        if(!$enc_id)
        {
            return redirect()->back();
        ...
        if($this->perform_unblock(base64_decode($enc_id)))
        {
            if($request->ajax()){
                ...
                $arr_resp['msg']    = str_singular($this->module_title). ' Inactivated Successfully';
                return $arr_resp;
            }else{
        ...
        else
        {
             if($request->ajax()){
                ...
                $arr_resp['msg']    = 'Problem Occured While '.str_singular($this->module_title).' Deactivation ';
                return $arr_resp;
            }else{
                Session::flash('error', 'Problem Occured While '.str_singular($this->module_title).' Activation ');
            }
    ...
    public function block($enc_id = FALSE,Request $request)
    {
        
        //dd($this->module_title);
        if(!$enc_id)
        {
            return redirect()->back();
        }
