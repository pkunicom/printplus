		...
		<div class="page-content">
			...
			<div class="content-wrapper">
				...
				<div class="add-user-btn-main">
					<a class="btn btn-primary"  id="open_add_city_modal" href="javascript:void(0)">Add City</a>
				</div>
						...
						<div id="add_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ $module_url_path }}/add_city" id="add_city" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
											...
											<div class="row">
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Country</label>
														<select class="form-control" name="add_country" id="add_country" data-rule-required="true">
															<option value="">Select Country</option>
															@foreach($arr_country as $key => $value)
																<option value="{{ $value['id'] ?? '' }}">{{ $value['country_english_name'] ?? ''}}</option>
															@endforeach
														</select>
													</div>
												</div>
												...
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Aramex City</label>
														<select class="form-control" name="add_aramex_city_id" id="add_aramex_city_id" >
						...
						<div id="edit_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ $module_url_path }}/edit_city" id="edit_city" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
										...
										<div class="modal-body">
											<div class="row">
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Country</label>
														<select class="form-control" name="edit_country" id="edit_country" data-rule-required="true">
												...
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Aramex City</label>
														<select class="form-control" name="edit_aramex_city_id" id="edit_aramex_city_id" >
															<option value="">Select Aramex City</option>
														</select>
													</div>
												</div>
...
$(function() {
    ...
    var table = $('.datatable-column-search-inputs').DataTable({
        ...
        columns: [
            ...
            {data : 'city_english_name',"orderable":false,"searchable":true,name:'city_english_name'},
            {data : 'city_arabic_name',"orderable":false,"searchable":true,name:'city_arabic_name'},
            {
...
$(document).ready(function(){
	...
	$('body').on('click','#open_add_city_modal',function(){
	...
	$('body').on('click','#open_edit_modal',function(){
		...
		$.ajax({
            url : '{{ $module_url_path }}/get_city_data/'+id,
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){
                    $('#enc_id').val(btoa(resp.data.id));
                    $('#edit_english_name').val(resp.data.city_english_name);
                    $('#edit_arabic_name').val(resp.data.city_arabic_name);
                    $('#edit_country').html(resp.data.country);
                    $('#edit_aramex_city_id').html(resp.data.aramex_city);
	...
	$('body').on('change','#add_country',function(){
        var id = $(this).val();
		$.ajax({
            url : '{{ $module_url_path }}/get_aramexcity_data/'+btoa(id),
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){
                    $('#add_aramex_city_id').html(resp.aramex_html);
                }
            }
        })	
	});

	$('body').on('change','#edit_country',function(){
        var id = $(this).val();
		$.ajax({
            url : '{{ $module_url_path }}/get_aramexcity_data/'+btoa(id),
            type : "GET",
            ...
            success:function(resp){
                if(resp.status=='success'){
                    $('#edit_aramex_city_id').html(resp.aramex_html);
                }
            }
        })	
	});
