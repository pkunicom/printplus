		...
		<div class="page-content">
			...
			<div class="content-wrapper">
				...
				<div class="add-user-btn-main">
					<a class="btn btn-primary"  id="open_add_staff_modal" href="javascript:void(0)">Add User</a>
				</div>					
						...
						<div id="modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ url('/') }}/agent/user/store_staff" id="add_staff" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
						...
						<div id="edit_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ url('/') }}/agent/user/update" id="edit_staff" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
...
$(function() {
    ...
    // });
    // Agent staff listing : Author(Harsh Chauhan)
...
$(document).ready(function(){
	...
	// Code to get countries Author : (Harsh Chauhan)
	$('body').on('click','#open_add_staff_modal',function(){
		 $.ajax({
            url : '{{ url('/') }}/agent/user/get_countries',
            type : "GET",
            dataType: 'JSON',
	...
	//  Open edit staff Modal Author : HArsh Chauhan
	$('body').on('click','#open_edit_staff_modal',function(){
        var id = btoa($(this).data("id"));
        $('.reset_password').hide();
        $.ajax({
            url : '{{ url('/') }}/agent/user/edit_get_countries/'+id,
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){
        ...
        // Edit Staff Appen data : Author (Harsh Chauhan)
		$.ajax({
            url : '{{ url('/') }}/agent/user/edit_staff/'+id,
            type : "GET",
            dataType: 'JSON',
            data : id,
            success:function(resp){
	...
	$('body').on('click','#block-unblock',function(){
		...
		if(status=='1'){
			var status_url = '{{ url('/') }}/agent/user/block/'+id
		}else{
			var status_url = '{{ url('/') }}/agent/user/unblock/'+id
		}
