				...
				<div class="content">
					...
					<div class="panel panel-flat">						
						...
						<div class="add-user-btn-main">
							...
							<a class="btn btn-primary"  id="open_add_discount_modal" href="javascript:void(0)">Add Discount</a>
						</div>
						...
						<div id="add_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ $module_url_path }}/add_discount" id="add_discount" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}								
										<div class="modal-body">								
											<div class="row">
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														...
														<input type="text" data-rule-required="true"  data-rule-number="true" name="add_percentage" id="add_percentage" class="form-control" >
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-6">
													<div class="form-group ">												
						...
						<div id="edit_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ $module_url_path }}/edit_discount" id="edit_discount" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}									
										<div class="modal-body">
											...
											<div class="row">
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Percentage</label>
														<input type="text" data-rule-required="true"  data-rule-number="true" name="edit_percentage" id="edit_percentage" class="form-control" >
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-6">
													<div class="form-group ">												
												...
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Category</label>
														<select class="form-control change_category" name="edit_category" id="edit_category"></select>
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Sub-Category</label>
														<select class="form-control change_sub_category" name="edit_sub_category" id="edit_sub_category"></select>
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>Product</label>
														<select class="form-control" name="edit_product" id="edit_product"></select>
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
												...
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">												
														<label>City</label>
														<select class="form-control" name="edit_city" id="edit_city"></select>
													</div>
												</div>
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">		
														<div class="checkbox">
															<label>
																<input type="checkbox"  id="edit_delivery_service" name="edit_delivery_service">
																Delivery Service
															</label>
														</div>
													</div>
												...
												<div class="col-sm-6 col-md-6 col-lg-12">
													<div class="form-group">		
														<div class="checkbox">
															<label>
																<input type="checkbox" id="edit_limit_customer" name="edit_limit_customer">
																Limit this discount for first order for new customers
															</label>
														</div>
													</div>
...
$(function() {
    ...
    var table = $('.datatable-column-search-inputs').DataTable({
        ...
        columns: [         
            ...
            {data : 'discount_id',"orderable":true,"searchable":true,name:'discount_id'},
...
$(document).ready(function(){
	...
	$('body').on('click','#open_add_discount_modal',function(){
    	$.ajax({
            ...
            success:function(resp){
                if(resp.status=='success'){              	
                	$('#add_category').html(resp.data.category);
                	$('#add_sub_category').html(resp.data.sub_category);
                	$('#add_product').html(resp.data.product);
    ...
    $('body').on('change','.change_category',function(){
    	...
    	$.ajax({
            url : '{{ url('/') }}/admin/discounts/get_category_selected_data/'+btoa(category_id),
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){             	
                	$('#add_sub_category').html(resp.data.sub_category);
                	$('#add_product').html(resp.data.product);
                	$('#edit_sub_category').html(resp.data.sub_category);
                	$('#edit_product').html(resp.data.product);           	
                }else if(resp.status=='error'){
                    $("#edit_modal_form_vertical").modal('hide');
                }
            }
     ...
     $('body').on('change','.change_sub_category',function(){
    	var sub_category_id = $(this).val(); 
    	$.ajax({
            url : '{{ url('/') }}/admin/discounts/get_subcategory_selected_data/'+btoa(sub_category_id),
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){             	             	
	...
	$('body').on('click','#open_edit_modal_form_vertical',function(){
		...
		$.ajax({
            url : '{{ url('/') }}/admin/discounts/get_discount_data/'+id,
            type : "GET",
            dataType: 'JSON',
            success:function(resp){
                if(resp.status=='success'){
                    $('#enc_id').val(btoa(resp.data.discount.id));
                    $('#edit_percentage').val(resp.data.discount.percentage);
                    $('#edit_start_date').val(resp.data.discount.start_date);
                    $('#edit_start_time').val(resp.data.discount.start_time);
                    $('#edit_end_date').val(resp.data.discount.end_date);
                    $('#edit_end_time').val(resp.data.discount.end_time);
                    $('#edit_category').html(resp.data.category);
                    $('#edit_sub_category').html(resp.data.sub_category);
                    $('#edit_product').html(resp.data.product);
                    $('#edit_country').html(resp.data.sys_country);
                    $('#edit_city').html(resp.data.sys_city);
                    if(resp.data.discount.delivery_service=='yes'){
                  		$("#edit_delivery_service"). prop("checked", true);
                    }else{
                  		$("#edit_delivery_service"). prop("checked", false);
                    }
                    if(resp.data.discount.first_order_new_customer=='yes'){
                  		$("#edit_limit_customer"). prop("checked", true);
                    }else{
                  		$("#edit_limit_customer"). prop("checked", false);
                    }
