				...
				<div class="content category-table-main-block">
					...
					<div class="panel">
						...
						<div class="add-user-btn-main">
							...
							<a class="btn btn-primary"  id="open_add_sub_category_modal" href="javascript:void(0)">Add Sub-Category</a>
						</div>
					...
					<div id="modal_form_vertical" class="modal fade addUserModalMain">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<form action="{{ url('/') }}/admin/category/store_sub_category" id="add_sub_category" method="post" enctype="multipart/form-data" >
									{{csrf_field()}}
									<div class="modal-body">
									<input type="hidden" name="category_id" value="{{isset($arr_data['id'])? $arr_data['id'] :'-'}}">
										<div class="row">
					...
					<div id="edit_modal_form_vertical" class="modal fade addUserModalMain">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<form action="{{ url('/') }}/admin/category/update_sub_cat" id="edit_category" method="post" enctype="multipart/form-data" >
									{{csrf_field()}}
					...
					<div class="panel">										
						<table border = "1" class="table">
							...
							<tbody>
								<td style="text-align: center;"> {{isset($arr_data['id'])? $arr_data['id'] :'-'}}</td>
...
$(document).ready(function(){
	...
	$('body').on('click','#open_add_sub_category_modal',function(){
		$("#modal_form_vertical").modal('show');	
	});
	$('body').on('click','#open_edit_sub_category_modal',function(){
        var id = btoa($(this).data("id"));
		$.ajax({
            url : '{{ url('/') }}/admin/category/edit_sub_cat/'+id,
            type : "GET",
            dataType: 'JSON',
            data : id,
		...
		$.ajax({
            ...
            success:function(resp){
                if(resp.status=='success'){
					...
					$('#view_category_id').val(resp.data.category_id);
                }else if(resp.status=='error'){
...
function check_en_category()
{
	...
	var base_url                = '{{$module_url_path}}/en_sub_category_check';
	$.ajax({
		url: base_url,
		type: "POST",
		data: {
		...
		success:function(resp){
				...
				{ 
					document.getElementById('error_en_name').innerHTML = 'Sub-Category allready Exist';
...
function check_ar_category()
{
	...
	// alert(ar_category);
	var token                   = "{{csrf_token()}}";
	var base_url                = '{{$module_url_path}}/ar_sub_category_check';
	$.ajax({
		url: base_url,
		type: "POST",
		data: {
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_ar_name').innerHTML = 'Sub-Category allready Exist';
					$("#proceed_add").attr('disabled', true);
					return false;
				}
				else
...
function check_en_edit_category()
{
	...
	$.ajax({
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_edit_english_name').innerHTML = 'Sub Category allready Exist';
					$("#proceed_edit").attr('disabled', true);
					return false;
				}
				else
...
function check_ar_edit_category()
{
	...
	$.ajax({
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_edit_arabic_name').innerHTML = 'Sub Category allready Exist';
					$("#proceed_edit").attr('disabled', true);
					return false;
				}
				else
...
$(function() {
    ...
    var table = $('.datatable-column-search-inputs').DataTable({
	    ...
	    "bLengthChange": true,
        ajax: {
            url: "{{ $module_url_path}}/load_sub_category_data",
            data: function(d) {
				...
				d['column_filter[cat_id]']= "{{$arr_data['id']}}";
            }
        },
        columns: [
            {data : 'sr_no',"orderable":false,"searchable":false,name:'sr_no'},
			{data : 'cat_subcat_ID',"orderable":false,"searchable":true,name:'cat_subcat_ID'},
