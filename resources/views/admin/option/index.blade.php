				...
				<div class="content">
					...
					<div class="panel panel-flat">						
						...
						<div class="add-user-btn-main">
							...
							<a class="btn btn-primary"  id="open_add_option_modal" href="javascript:void(0)"><i class="icon-plus3"></i> Add Option</a>
						</div>
						<div class="clearfix"></div>
						<div id="modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ url('/') }}/admin/option/store_option" id="add_option" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
											...
											<div class="row">
												<div class="col-sm-12 col-md-12 col-lg-12">
													<div class="form-group">											
														...
														<input type="text" data-rule-required="true" name="en_name" id="en_name" placeholder="Enter English Option Name" class="form-control" maxlength="100" oninput="check_en_option(this)">
													</div>
												...
												<div class="col-sm-12 col-md-12 col-lg-12">
													<div class="form-group">												
														...
														<input type="text" data-rule-required="true"  name="ar_name" id="ar_name"  placeholder="Enter Arabic Option Name" class="form-control" maxlength="100" oninput="check_ar_option(this)">
													</div>	
						...
						<div id="edit_modal_form_vertical" class="modal fade addUserModalMain">
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<form action="{{ url('/') }}/admin/option/update" id="edit_option" method="post" enctype="multipart/form-data" >
										{{csrf_field()}}
...
$(document).ready(function(){
	...
	$('body').on('click','#open_edit_option_modal',function(){
        var id = btoa($(this).data("id"));
		$.ajax({
            url : '{{ url('/') }}/admin/option/edit/'+id,
            type : "GET",
            dataType: 'JSON',
            data : id,
	...
	$('body').on('click','#open_view_option_modal',function(){
        var id = btoa($(this).data("id"));
		$.ajax({
            url : '{{ url('/') }}/admin/option/view/'+id,
            type : "GET",
            dataType: 'JSON',
            data : id,
            success:function(resp){
                if(resp.status=='success'){
					...
					$('#view_option_id').val(resp.data.option_id);                  
                }else if(resp.status=='error'){
...
function check_en_option()
{
	...
	var base_url                = '{{$module_url_path}}/en_option_check';
	$.ajax({
		url: base_url,
		type: "POST",
		data: {
		...
		success:function(resp){
				...
				{ 
					document.getElementById('error_en_name').innerHTML = en_option+' Option allready Exist';
...
function check_ar_option()
{
	...
	// alert(ar_option);
	var token                   = "{{csrf_token()}}";
	var base_url                = '{{$module_url_path}}/ar_option_check';
	$.ajax({
		url: base_url,
		type: "POST",
		data: {
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_ar_name').innerHTML = ar_option+' Option allready Exist';
					$("#proceed_add").attr('disabled', true);
					return false;
				}
				else
...
function check_en_edit_option()
{
	...
	$.ajax({
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_edit_english_name').innerHTML = 'option allready Exist';
...
function check_ar_edit_option()
{
	...
	$.ajax({
		...
		success:function(resp){
				if(resp.status=='error')
				{ 
					document.getElementById('error_edit_arabic_name').innerHTML = 'option allready Exist';
					$("#proceed_edit").attr('disabled', true);
					return false;
				}
				else
...
$(function() {
    ...
    var table = $('.datatable-column-search-inputs').DataTable({
        ...
        columns: [         
            ...
            {data : 'option_id',"orderable":false,"searchable":true,name:'option_id'},
